...
impl Cpu {
        ...
        Self: IO8<S>,
    {
        step!((), {
            0: if let Some(v) = self.read8(bus, src) {
                ...
                let result = self.regs.a.wrapping_add(v).wrapping_add(cf);
                self.regs.set_zf(result == 0);
                self.regs.set_nf(false);
                self.regs.set_hf((self.regs.a & 0xf) + (v & 0xf) + (cf & 0xf) > 0xf);
                self.regs.set_cf(self.regs.a as u16 + v as u16 + cf as u16 > 0xff);
                self.regs.a = result;
                self.fetch(bus);
            },
        });
    ...
    pub fn res<S: Copy>(&mut self, bus: &mut Peripherals, num: u8, src: S)
    where
        Self: IO8<S>,
    {
        step!((), {
    ...
    pub fn callc(&mut self, bus: &mut Peripherals, c: Cond) {
        step!((), {
            0: if let Some(v) = self.read16(bus, Imm16) {
                if !self.cond(c) {
                    self.fetch(bus);
